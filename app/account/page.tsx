      ...
      fetch(`/api/account/verify?token=${encodeURIComponent(token)}`)
        ...
        .then(data => {
          if (data.sessionToken && data.email) {
            ...
            setError(data.error || 'Link is verlopen of ongeldig')
          }
        })
    ...
    fetch('/api/account/orders', {
      ...
      .then(data => {
        if (data.orders) {
          ...
          setError(data.error || 'Sessie verlopen')
    ...
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="max-w-md mx-auto">
          ...
          <p className="text-gray-600 mb-8">Log in met je e-mailadres. Je krijgt een inloglink toegestuurd.</p>

          {sent ? (
            <div className="bg-green-50 rounded-lg p-6 text-center">
              <h2 className="text-xl font-semibold text-green-900 mb-2">Check je inbox!</h2>
              <p className="text-green-800">
                We hebben een inloglink gestuurd naar <strong>{email}</strong>. De link is 1 uur geldig.
              </p>
            </div>
          ) : (
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      {loading && orders === null ? (
        ...
        <div className="bg-white rounded-lg shadow p-8 text-center">
          <h2 className="text-xl font-semibold text-gray-900 mb-2">Nog geen bestellingen</h2>
          <p className="text-gray-600 mb-6">Zodra je iets bestelt, zie je het hier met de status en trackingcode.</p>
            ...
            <div key={order.id} className="bg-white rounded-lg shadow p-6">
              ...
              {order.tracking_code && (
                <div className="bg-blue-50 rounded-lg p-3">
                  <span className="font-medium text-blue-900">Trackingcode: </span>
  ...
  return (
    <Suspense fallback={
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 text-center">
        <h1 className="text-3xl font-bold text-gray-900 mb-4">Mijn account</h1>
        <p className="text-gray-600">Laden...</p>
      </div>
    }>
      <AccountContent />
